...
public class ClientCtrl extends AbstractControllerResource {
    ...
    public Client delete(Client a) throws Exception {
        Client verify = getWithId(a.getResourceId());
        if (verify != null) {
            TypedQuery<Client> q = getEntityManagerFactory().createEntityManager().createQuery("delete from Client where resourceId = ?1", Client.class);
    ...
    public Client update(Client a) throws Exception {
        Client verify = getWithId(a.getResourceId());
        if (verify != null) {
            TypedQuery<Client> q = getEntityManagerFactory().createEntityManager().createQuery("update Client set firstname = ?2, lastname = ?3, login = ?4, password = ?5 where resourceId = ?1", Client.class);
            q.setParameter(1, a.getResourceId());
            q.setParameter(2, a.getFirstName());
            q.setParameter(3, a.getLastName());
            q.setParameter(4, a.getLogin());
            q.setParameter(5, a.getPassword());
        } else {
            throw new Exception("This account doesn't exist");
        }
        return a;
